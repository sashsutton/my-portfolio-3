...
export const shared = {
  ...
  /**
   * Tracks. `src` is an optional path under public/ — set it and the play
   * button drives real audio; leave it null and the button just spins the
   * platter.
   * TODO: these three titles/lengths are still placeholders. The real music
   * now lives in the SoundCloud embed below (shared.media.soundcloud), so
   * either replace these with your real track names or drop the fake tracklist.
   */
  tracks: [
    { id: "a1", no: "A1", title: "Untitled Set — live", len: "48:12", src: null },
    { id: "a2", no: "A2", title: "Sound of the room", len: "05:37", src: null },
    { id: "a3", no: "A3", title: "Guitar sketch #4", len: "03:02", src: null },
  ],

  /**
   * Real, embedded media on the /music page.
   *  - `soundcloud` is the profile/playlist URL; the component wraps it in the
   *    SoundCloud widget so the tracks play inline.
   *  - the YouTube video is stored as id + start seconds (from the ?t=10s in
   *    the share link) so the embed URL can be built cleanly.
   */
  media: {
    soundcloud: "https://soundcloud.com/sasha-sutton-974644697/popular-tracks",
...
const en = {
  ...
  music: {
    ...
    side: { title: "Side A" },
    trackMeta: {
      a1: "electronic · dj mix",
      a2: "production · original",
      a3: "guitar · demo",
    },
    ...
    play: "Play",
    pause: "Pause",
    soundcloudLabel: "// full tracks on soundcloud",
    video: {
      title: "In motion",
      // TODO: replace — written without seeing the clip. Say what it is.
      caption: "A set, filmed.",
    },
    credits: {
      title: "Credentials",
      items: [
...
const fr = {
  ...
  music: {
    ...
    play: "Lecture",
    pause: "Pause",
    soundcloudLabel: "// morceaux complets sur soundcloud",
    video: {
      title: "En mouvement",
      caption: "Un set, filmé.",
    },
    credits: {
      title: "Parcours",
      items: [
