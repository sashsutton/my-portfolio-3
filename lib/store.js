...
/**
 * Handle to the /music radio-set <audio> element, so the floating deck control
 * can drive the same player that shows inline in the Sets section. The <audio>
 * registers itself here; the transport reads it. A plain mutable ref, not
 * state — nothing needs to re-render when it is set.
 */
