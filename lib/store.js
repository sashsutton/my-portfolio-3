...
/**
 * The SoundCloud Widget instance on /music, once its API has bound to the
 * embed. The Sets section creates it; the floating deck reads it so its button
 * can drive the same player. A plain mutable ref — nothing re-renders on it.
 */
export const scWidgetRef = { current: null };
...
export const useAppStore = create((set) => ({
  ...
  /** Title of the current SoundCloud track, read live from the widget so the
   *  deck shows what is actually playing. Empty until the first play. */
  trackTitle: "",
